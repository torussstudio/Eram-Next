  ...
  return (
    <div className="flex flex-col items-center gap-2.5">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div
          className="absolute inset-[7px] rounded-full border border-dashed flex items-center justify-center"
      ...
      <div className="text-center leading-tight">
        <p className="font-rethink text-[11px] font-medium tracking-wide text-[#2b2620]">
          {p.label}
        </p>
        <p
...
export default function AdminDashboard() {
 ...
 useEffect(() => {
  ...
  async function loadAll() {
    ...
    try {
      const academicsRequests = ACADEMIC_SCHOOLS.map((school) =>
        api
          .get("/academics", { params: { school } })
          .catch(() => ({ data: [] })),
      );

      const [galleryRes, eventsRes, downloadsRes, ...academicsRes] =
        await Promise.all([
  ...
  // Tab back aayi focus cheyyumbolum, page visible aavumbolum re-fetch cheyyuka
  const handleRefetch = () => {
  ...
  const pulse = useMemo(() => {
    const counts = INSTITUTIONS.map((inst) => ({
      ...inst,
      count:
        gallery.filter((g) => g.category === inst.id).length +
  ...
  const areaPath = trendPoints.length
    ? `${linePath} L ${trendPoints[trendPoints.length - 1].x.toFixed(
        1,
      )} 120 L 0 120 Z`
    : "";
    ...
    <div className="min-h-screen bg-[#F5EFE8]">
      <div className="mx-auto max-w-7xl px-6 py-10 sm:py-12">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className="font-rethink text-[11px] uppercase tracking-[0.28em]"
                  style={{ color: GOLD }}
                >
            ...
            <div className="text-right">
              <p className="font-rethink text-[10px] uppercase tracking-[0.2em] text-[#b5aa98]">
                Today
              </p>
              <p className="font-display text-sm text-[#2b2620] mt-0.5">
                {today}
              </p>
            </div>
          </div>
          ...
          {stats.map((s) => (
            ...
            >
              ...
              <div className="flex items-start justify-between">
                ...
                <span className="font-rethink text-[9px] uppercase tracking-widest text-[#d8cdb8]">
                  Total
                </span>
              </div>
              <p className="mt-4 font-display text-3xl tabular-nums text-[#2b2620]">
                {s.value}
              </p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          ...
          <div className="rounded-2xl border border-[#e3d6c3] bg-white p-4 sm:p-6 shadow-[0_1px_2px_rgba(43,38,32,0.04)]">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-6">
              <div>
                <h2 className="font-display text-base text-[#2b2620]">
                  Content Activity
                </h2>
                <p className="font-rethink text-xs text-[#8a7f6f] mt-0.5">
                  Uploads &amp; posts, last 7 days
                </p>
              </div>
              <span className="shrink-0 rounded-full border border-[#e3d6c3] px-2.5 py-1 font-rethink text-[10px] uppercase tracking-wide text-[#8a7f6f] whitespace-nowrap">
                All institutions
              </span>
            </div>
            ...
            <div className="flex justify-between mt-1 gap-0.5">
              {trend.map((t, i) => (
                ...
                >
                  <span className="font-display text-[11px] sm:text-xs tabular-nums text-[#2b2620]">
                    {t.count}
                  </span>
                  <span className="font-rethink text-[8px] sm:text-[10px] uppercase tracking-tight sm:tracking-widest text-[#b5aa98]">
                    {t.label}
                  </span>
                </div>
              ))}
          ...
          {/* Activity feed */}
          <div className="rounded-2xl border border-[#e3d6c3] bg-white p-4 sm:p-6 shadow-[0_1px_2px_rgba(43,38,32,0.04)]">
            <h2 className="font-display text-base text-[#2b2620] mb-5">
              Recent Activity
            </h2>
                    ...
                    <div key={i} className="flex gap-3.5 relative">
                      ...
                      <div className="min-w-0 flex-1 pb-0.5">
                        <div className="flex items-start sm:items-center justify-between gap-2 sm:gap-3">
                          <p className="min-w-0 flex-1 font-rethink text-sm text-[#2b2620] flex items-center gap-1.5">
                          ...
                          <span className="shrink-0 font-rethink text-[10px] text-[#b5aa98] whitespace-nowrap">
                            {timeAgo(a.date)}
                          </span>
                        </div>
                        <p className="font-rethink text-[11px] text-[#8a7f6f] mt-0.5 capitalize truncate">
