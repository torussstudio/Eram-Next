...
//     role:
//       "Secretary,\nEram Educational And Welfare Trust (AMLP)",
//     num: "02",
...
function StructuredLearningSection() {
  ...
  // Arrow scroll handlers — one card per click
  const scrollByCard = useCallback((direction) => {
    const el = scrollRef.current;
    if (!el) return;
    const card = el.querySelector(".structured-card");
    ...
    >
      <div className="w-full md:max-w-[1200px] md:mx-auto md:px-8 lg:px-12">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          <div className="structured-scroll-container -mx-5 md:mx-0 overflow-hidden">
            {/* Arrow Buttons */}
            <div className="flex justify-end gap-2 px-5 md:px-0 mb-3">
              <button
                onClick={() => scrollByCard(-1)}
                aria-label="Scroll left"
                className="w-9 h-9 rounded-full border border-white/50 flex items-center justify-center hover:bg-white hover:text-[#b5122b] transition-colors cursor-pointer"
              >
                <ChevronLeft size={18} />
              </button>
              <button
