...
//               </span>
//               <div
//                 className="systems-eyebrow-line h-px bg-[#F5EFE8] w-10"
...
function SystemsThatSustainExcellence() {
  ...
  useGSAP(
    () => {
      ...
      // ── SHARED: heading reveal ──────────────────────────────────────
      // FIX 1: "all" is not a valid GSAP matchMedia query string.
      // Use "(min-width: 0px)" so it always matches every device.
      mm.add("(min-width: 0px)", () => {
        // FIX 2: Left and right eyebrow lines need separate animations so
        // the right line expands from "right center" instead of being
        // overridden to "left center" by the shared selector.
        gsap.fromTo(
      ...
      const buildStackTimeline = (scrollEndPx) => {
        ...
        for (let step = 0; step < total; step++) {
          ...
          // Background word crossfade
          // FIX 3: The original code had a duplicate tl.fromTo AFTER the loop
          // that targeted the last bg word at the exact same timeline position
          // as the in-loop fade-in — causing a direct opacity conflict on the
          // same element at the same time. Fix: remove the post-loop duplicate.
          // Instead, each word fades in for the first half of its segment and
          // fades out for the second half, giving a clean crossfade with no
          // overlap conflicts on any device.
