...
// Google Maps embed (no API key needed — uses the classic /maps?output=embed form)
const MAP_EMBED_SRC =
  "https://www.google.com/maps?q=Eram+Education,+Eram+Nagar,+Prabhapuram,+Mannengode,+Palakkad+679307&output=embed";
...
// Google Maps embed block — rounded card matching the site's design language,
// with a subtle grayscale-to-color reveal on hover for a premium feel.
function MapEmbed() {
  return (
    <a
      href="https://maps.google.com/?q=Eram+Education+Palakkad"
      ...
      className="map-embed block relative w-full h-[190px] sm:h-[210px] rounded-2xl overflow-hidden
        border border-white/10 group cursor-pointer"
    >
      <iframe
        src={MAP_EMBED_SRC}
        title="Eram Education location on Google Maps"
        width="100%"
        height="100%"
        style={{ border: 0 }}
        loading="lazy"
      ...
      />
      {/* Accent overlay so the map reads as part of the dark card, not a raw embed */}
      <div className="absolute inset-0 bg-[#ae1431]/10 mix-blend-multiply pointer-events-none" />
      <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-3 py-1.5 rounded-full
        bg-black/70 backdrop-blur-sm text-white text-[11px] tracking-[0.04em] font-rethink">
...
export default function ContactPage() {
  ...
  useGSAP(
    () => {
      ...
      // ── 2b. Reveal for the map embed ────────────────────────────────
      gsap.set(".map-embed", { opacity: 0, y: 16, scale: 0.98 });
      gsap.to(".map-embed", {
        opacity: 1,
        y: 0,
        scale: 1,
