...
export default function GalleryClient() {
  ...
  const [activeType, setActiveType] = useState<TypeId | "all">(() => {
    ...
    return fromUrl && VALID_TYPE_IDS.includes(fromUrl)
      ? (fromUrl as TypeId)
      : "all";
  });
  ...
  return (
    <main className="font-rethink min-h-screen bg-[#F5EFE8]">
      ...
      <section
        ref={heroRef}
        className="px-6 pt-8 pb-6 md:px-12 lg:px-20 lg:pt-10"
      >
        <div className="relative flex flex-col md:flex-row items-stretch overflow-hidden rounded-2xl bg-[#ae1431] min-h-[300px]">
          ...
          <div className="relative z-10 flex-1 flex flex-col justify-center px-8 py-10 md:px-12 md:py-12">
            ...
            <h1 className="hero-reveal font-display mt-3 text-4xl leading-[1.05] text-white md:text-5xl lg:text-6xl">
              Capturing Life at ERAM
            </h1>
            <p className="hero-reveal mt-5 max-w-md leading-relaxed text-white/80 font-rethink text-sm md:text-base">
              A visual record of sport, culture, service, and scholarship
              across every institution in the ERAM ecosystem.
            </p>
            <div className="hero-reveal mt-6 h-[2px] w-16 bg-white/40" />
          </div>
