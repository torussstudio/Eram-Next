...
export default function ArenaSection() {
  ...
  useGSAP(
    () => {
      ...
      gsap.set(leftColRef.current, { x: -36 });
      gsap.set(rightPanelRef.current, { x: 36 });
      gsap.set(eramTextRef.current, { opacity: 0, y: 20 });
  ...
  return (
    <section
      ref={sectionRef}
      id="arena"
      className={`${section} ${sectionBand} bg-[#ae1431] min-h-screen -mb-[1px]`}
      style={{ transform: "translateZ(0)" }} // ✅ no height:100vh
    >
      <div
        className={`
          ${shell}
      ...
      >
        ...
        <div
          ref={arenaOutlineRef}
          className="absolute z-[20] left-[30px] bottom-[-55px] text-[clamp(7rem,13vw,14rem)] font-bold text-transparent [-webkit-text-stroke:1.6px_black]"
        >
          SPORTS ARENA
        </div>
        ...
        <div
          ref={gridRef}
          className="relative z-[20] grid gap-[40px] [grid-template-columns:0.85fr_1.15fr]"
        >
          {/* LEFT */}
          <div ref={leftColRef}>
            ...
            <p className="mb-[34px] text-[14.5px]">
              The ERAM Sports Arena reflects the Trust's long-term commitment to
          ...
          <div className="relative min-h-[520px] overflow-hidden rounded-[24px]">
            ...
            <div
              ref={eramTextRef}
              className="absolute right-[20px] bottom-[10px] text-white text-[3rem] font-bold"
            >
              THE ERAM
            </div>
          </div>
