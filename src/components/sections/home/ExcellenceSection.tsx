...
// /* ── ImageCard ───────────────────────────────────────────────────── */
// function ImageCard({ src, className = "", cardClass = "" }: ImageCardProps) {
//   return (
//     <div
...
export default function ExcellenceSection() {
  ...
  const sectionRef = useRef<HTMLDivElement>(null);
  const isAnimating = useRef(false);
  // true only after the scroll-entrance animation has completed at least once
  ...
  useEffect(() => {
    // Only run after the scroll-entrance animation has finished.
    // entranceDone is flipped to true inside the onComplete callbacks below.
  ...
  return (
    ...
    >
      {/* ── Desktop ≥ 900px ─────────────────────────────────────── */}
      <div className="mx-auto hidden max-w-[1180px] grid-cols-[240px_1fr] gap-16 px-6 min-[900px]:grid">
        {/* Sidebar — self-start required for sticky inside a grid */}
        <aside className="sticky top-[120px] self-start flex flex-col gap-5 pt-5">
          {CATEGORIES.map((label, i) => (
            <button
              key={label}
