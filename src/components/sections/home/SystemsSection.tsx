...
//       const section = document.getElementById("structured-learning");

//       if (section) {
//         smoothScrollTo("structured-learning");
//       } else if (attempts < 30) {
//         setTimeout(() => scrollToStructured(attempts + 1), 100);
//       }
//     };

//     setTimeout(() => scrollToStructured(), 300);
//   }}
//   className="font-rethink rounded-[8px] border border-white px-[36px] py-[14px] text-[12px] uppercase tracking-[0.16em] text-white transition-all duration-300 hover:border-white hover:bg-white hover:text-[#a80c2d] cursor-pointer flex items-center justify-center gap-2"
// >
...
export default function SystemsSection() {
  ...
  const goToStructuredLearning = useCallback(() => {
    ...
    const scrollToStructured = (attempts = 0) => {
      ...
      const waitForStableLayout = (tries = 0) => {
        ...
        // 3 consecutive checks with no height change = layout settled
        if (stableCount >= 3 || tries >= 20) {
          smoothScrollTo("structured-learning");
          return;
        }

        setTimeout(() => waitForStableLayout(tries + 1), 150);
      };
