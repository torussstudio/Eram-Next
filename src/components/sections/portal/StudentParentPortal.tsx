...
function getResourceIcon(fileType: string) {
  ...
  if (t.includes("DOC"))
    return <FileQuestion className="text-blue-700 shrink-0" size={22} />;
  if (t.includes("XLS"))
    return <BookOpen className="text-green-700 shrink-0" size={22} />;
  if (t.includes("PNG") || t.includes("JPG") || t.includes("JPEG"))
    return <FileText className="text-purple-700 shrink-0" size={22} />;
  return <FileText className="text-red-700 shrink-0" size={22} />;
}
            ...
            <div className="space-y-3">
              {filteredResources.length > 0 ? (
                filteredResources.map((resource) => (
                  <div
                    key={resource.id}
                    className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5 flex items-center gap-4 hover:shadow-sm transition-shadow group"
                  >
