...
export default function BeyondAcademics({ school }: BeyondAcademicsProps) {
  ...
  useGSAP(
    () => {
      ...
      // Only animate stats whose value is purely numeric — text like
      // "Govt. Aided" is rendered as-is and skipped here.
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            (q(".counter-num") as HTMLElement[]).forEach((el) => {
              ...
              if (!/^\d+$/.test(raw.trim())) return; // skip non-numeric values
               ...
               const bg = isRed
                ...
                return (
                  <div key={card._id} className={`anim-excel p-7 ${bg}`}>
                    ...
                    {card.image && (
                      <div className="relative h-[180px] w-full rounded-xl overflow-hidden mb-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 rounded-2xl overflow-hidden">
              {stats.map((stat, i) => {
                ...
                return (
                  <div
                    key={stat._id}
                    className={`anim-stat px-8 py-10 flex flex-col justify-between ${bg}`}
                  >
                    <div className="flex items-baseline gap-1 mb-4">
                      <span
                        data-target={stat.value}
                        className="counter-num text-[56px] font-display sm:text-[64px] leading-none tracking-[-0.02em] text-white"
                      >
