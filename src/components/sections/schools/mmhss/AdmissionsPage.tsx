...
const btn2Ref = useRef<HTMLButtonElement>(null);
  useGSAP(() => {
    ...
    hero
      ...
      .to(q(".anim-hero-btn"), { opacity: 1, y: 0, duration: 0.5, stagger: 0.1 }, "-=0.35");
    ...
    if (instWrap) {
      gsap
        ...
        .to(
          q(".anim-inst-card"),
          { opacity: 1, y: 0, duration: 0.65, stagger: { each: 0.08, ease: "power1.inOut" } },
          "-=0.2",
        );
    }
  },
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-start lg:justify-between gap-12">
          ...
          <div className="max-w-xl">
            <div className="anim-hero-tag flex items-center gap-3 mb-6">
              <span className="font-rethink text-[11px] tracking-widest uppercase text-[#ae1431] ">
                Admissions 2026–27
              </span>
            </div>
  ...
  >
    ...
    <span className="font-rethink relative z-10 group-hover:text-white transition-colors duration-300">
      ...
      {/* INSTITUTIONS */}
      <section className="anim-inst-wrap bg-[#F5EFE8] px-8 py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-1 rounded-2xl overflow-hidden">
            {institutions.map((inst, i) => {
              ...
              return isActive ? (
                ...
                >
                  <p className="text-[10px] tracking-widest font-rethink uppercase text-[#8a8278] group-hover:text-white/50 mb-3 transition-colors duration-200">
                    {inst.type}
                  </p>
                  <p className="font-display text-[#1a1209] group-hover:text-white text-xl  mb-1 transition-colors duration-200">
                    {inst.name}
                  </p>
                  <p className="text-xs text-[#6b6256] font-rethink group-hover:text-white/60 transition-colors duration-200">
                    {inst.desc}
                  </p>
                </Link>
              );
