...
export default function ParentPartnership() {
  ...
  useGSAP(
    () => {
      // Create a timeline mapped to the scroll position
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: containerRef.current,
          start: "top 80%", // Starts animation when section is 80% in view
          toggleActions: "play none none none",
        },
      });

      // Left Column Animations
      tl.from(".anim-subtitle", { y: 20, opacity: 0, duration: 0.8, ease: "power3.out" })
        .from(".anim-title", { y: 30, opacity: 0, duration: 1, ease: "power4.out" }, "-=0.6")
        .from(".anim-divider", { scaleX: 0, transformOrigin: "left center", duration: 0.8, ease: "power3.inOut" }, "-=0.6")
        .from(".anim-desc", { y: 20, opacity: 0, duration: 0.8, ease: "power3.out" }, "-=0.6")
        .from(".anim-bullet", { x: -15, opacity: 0, duration: 0.6, stagger: 0.1, ease: "power2.out" }, "-=0.4")
        .from(".anim-quote", { opacity: 0, duration: 1 }, "-=0.2")
        .from(".anim-btn", { y: 20, opacity: 0, duration: 0.8, ease: "power3.out" }, "-=0.6");

      // Right Column (WhatsApp mockup) Animations - starts slightly before left column finishes
      tl.from(".anim-mockup", { y: 40, opacity: 0, duration: 1, ease: "power4.out" }, "-=1.5")
        .from(".anim-mockup-header", { opacity: 0, duration: 0.6 }, "-=0.6")
        .from(".anim-msg", { y: 20, opacity: 0, duration: 0.5, stagger: 0.15, ease: "power3.out" }, "-=0.4");
    },
    { scope: containerRef } // Scoping ensures animations only happen inside this component
  );
    ...
    <section ref={containerRef} className={`${shell} bg-[#F5EFE8] overflow-hidden`}>
      <div className="w-full max-w-[1300px] mx-auto px-5 sm:px-8 md:px-10 lg:px-16 py-16 md:py-20 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 xl:gap-20 items-center">
          ...
          {/* LEFT COLUMN */}
          <div className="flex flex-col">
            <div className="flex items-center gap-3 mb-8 anim-subtitle">
              <p className="text-[10px] sm:text-[11px] tracking-[0.28em] text-[#ae1431] uppercase font-medium">
                Parent Partnership
              </p>
            </div>
            ...
            <h2
              className="anim-title font-serif text-[#1a1209] leading-[1.05] tracking-[-0.02em] 
              text-[26px] sm:text-[32px] md:text-[36px] lg:text-[40px] xl:text-[44px]"
            >
              Discipline &amp; Communication as Accountability
            </h2>
            ...
            <p className="anim-desc text-[14px] md:text-[15px] leading-[1.85] text-[#3d3128] max-w-[560px]">
              The institution maintains a mandatory WhatsApp-based communication
              system to ensure parents are continuously informed — making parent
              engagement institutional, not incidental.
            </p>
