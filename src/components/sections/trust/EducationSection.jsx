...
const Card = ({ variant = "white", label, title, meta = [], body, list, badge, children }) => {
  ...
  return (
    <div className={styles[variant]}>
      ...
      {body && Array.isArray(body)
        ? body.map((b, i) => (
            <p key={i} className={`mt-3 text-[14px] leading-[1.7] ${textColor}`}>{b}</p>
          ))
        : body && <p className={`mt-3 text-[14px] leading-[1.7] ${textColor}`}>{body}</p>}
      {list && (
        <ul className={`mt-4 space-y-2 text-[14px] list-disc pl-4 ${markerColor} ${textColor}`}>
      ...
      {badge && (
        ...
        >
          <p className="text-[14px] font-semibold">{badge.value}</p>
          <p className={`text-[12px] ${variant === "white" ? "text-white/80" : "text-white/70"}`}>
            {badge.label}
          </p>
        </div>
      )}
...
// ─── Pillar header ──────────────────────────────────────────────────
const PillarHeader = ({ num, label, title, subtitle, active, onClick, headerRef }) => (
  <div className="mt-[20px] mb-[20px]" ref={headerRef}>
    <div className="flex items-start justify-between">
      ...
      <button onClick={onClick} className="mt-[18px] text-[20px] cursor-pointer select-none">
        <span
          style={{
...
const AnimatedPanel = ({ children, visible }) => {
