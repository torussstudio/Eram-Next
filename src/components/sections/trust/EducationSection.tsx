...
interface AnimatedPanelProps {
  children: React.ReactNode;
  visible: boolean;
  panelRef?: (el: HTMLDivElement | null) => void; // NEW: expose wrapper height to parent
}

const AnimatedPanel = ({ children, visible, panelRef }: AnimatedPanelProps) => {
  ...
  return (
    <div ref={setWrapperRef} style={{ overflow: "hidden", height: 0, opacity: 0 }}>
    ...
    <div className="pb-[30px]">
      ...
      <div className="grid md:grid-cols-2 gap-[20px]">
        ...
        <Card
          ...
          image="/images/teachertraining.avif"
    ...
    <div className="pb-[30px]">
      ...
      <div className="grid md:grid-cols-2 gap-[20px]">
        ...
        <Card
          ...
          image="/images/rehabilitation.avif"
          imageAlt="Teacher Training"
...
export default function EducationSection({
  ...
  const panelRefs = useRef<(HTMLDivElement | null)[]>([]); // NEW: tracks each panel wrapper's live height
  ...
  const handlePillarClick = (i: number) => {
    ...
    if (el) {
      let top = el.getBoundingClientRect().top + window.scrollY - navOffset;

      // If another panel is currently open ABOVE this one, it's about to
      // collapse. Subtract its current height up front so we scroll straight
      // to the header's FINAL resting position — no second correction needed.
      if (active !== null && active < i) {
        const collapsingHeight = panelRefs.current[active]?.offsetHeight ?? 0;
        top -= collapsingHeight;
      }
    ...
    setActive(i);
    // No setTimeout correction scroll — a single accurate scroll is enough now.
  };
