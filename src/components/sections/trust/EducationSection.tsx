...
interface AnimatedPanelProps {
  children: React.ReactNode;
  visible: boolean;
  // NEW — lets the parent know exactly when the collapse tween finishes,
  // so it can safely open the next panel only after this one is fully closed.
    ...
    if (visible) {
      ...
      tweenRef.current = gsap.fromTo(
        ...
        { height: currentH, opacity: 1 },
        {
          ...
          ease: "power3.in",
          onComplete: () => {
            // NEW — notify parent so it can trigger the next panel's open
            onCollapseComplete?.();
          },
        },
      );
...
export default function EducationSection({
  ...
  // NEW — tracks the index of the panel currently mid-collapse (if any).
  // While a panel is closing, we don't let a new one open — this is what
  // prevents the "double animation" height jump.
  const [closingIndex, setClosingIndex] = useState<number | null>(null);
  // NEW — remembers which panel the user actually wants open next,
  // so we can open it as soon as the current collapse finishes.
  const pendingTargetRef = useRef<number | null>(null);

  // NEW — central click handler for all pillar headers.
  const handleHeaderClick = (i: number) => {
    if (active === i) {
      // Clicking the currently open pillar just closes it. No sequencing needed.
      setActive(null);
      return;
    }

    if (active !== null) {
      // Another pillar is open: collapse it first, and only open the
      // newly clicked one once that collapse animation has finished.
      pendingTargetRef.current = i;
      setClosingIndex(active);
      setActive(null);
    } else {
      // Nothing open yet: open directly, no sequencing needed.
      setActive(i);
    }
  };

  // NEW — called by AnimatedPanel exactly when its collapse tween completes.
  const handleCollapseComplete = (i: number) => {
    if (closingIndex !== i) return; // stale callback from an unrelated panel
          ...
          return (
            <div key={i} id={`pillar-${i + 1}`}>
              <PillarHeader
                {...p}
                active={isActive}
                onClick={() => handleHeaderClick(i)} // CHANGED — routed through sequencing logic
                headerRef={(el) => {
                  headerRefs.current[i] = el;
                }}
              />
              <AnimatedPanel
                visible={isActive}
                onCollapseComplete={() => handleCollapseComplete(i)} // NEW
              >
                <Panel />
              </AnimatedPanel>
            </div>
