...
export default function PageTransition() {
  ...
  useLayoutEffect(() => {
    ...
    const handleNavbarTransition = (e: Event) => {
      ...
      // IMPORTANT: keep the logo hidden until the cream panels have
      // fully covered the screen. Revealing it earlier (while the
      // panels are still sliding in) lets the mark / faded guide-text
      // flash over the real page underneath, before the page is
      // actually covered.
      gsap.set(".logo-holder", {
        opacity: 0,
        scale: 1,
      ...
      tl.to(".left-panel", {
        ...
        // ── screen is now fully covered by the cream panels ──
        // Only now do we reveal the logo and start the classic
        // ink-fill sequence: mark -> eram fill -> education fill.
        .set(".logo-holder", {
          opacity: 1,
        })
        .to(".eram-mark", {
      ...
      // total time before navigate = panel cover 0.55s + mark 0.45s
      // + eram fill 0.7s + education fill 0.8s = 2.5s
      // (no more overlap with panel-close — the blank cream page is
      // fully in place before the logo/text starts revealing)
    };
